"""
Teste real com 3 passageiros para validar API MinasTaxi com otimização de rota
"""
import os
import sys
...
def test_3_passengers_optimized():
    """Testa envio de 3 passageiros com otimização de rota para API MinasTaxi"""
    
    print("\n" + "="*60)
    print("🧪 TESTE: 3 PASSAGEIROS - ROTA OTIMIZADA - API MINASTAXI")
    print("="*60)
    ...
    try:
        ...
        for idx, p in enumerate(optimized_passengers, 1):
            if destination_coords and 'lat' in p:
                ...
                print(f"   {idx}. {p['name']} (distância até destino: {dist:.1f}km)")
            else:
                print(f"   {idx}. {p['name']}")
        ...
        if order.passengers and 'lat' in order.passengers[0]:
            ...
            order.pickup_address = f"Múltiplas paradas: {' → '.join(addresses[:2])}" + ("..." if len(addresses) > 2 else "")
        else:
        ...
        print("\n" + "="*60)
        print("📋 DADOS PRONTOS PARA ENVIO (ROTA OTIMIZADA):")
        print("="*60)
        print(f"Passageiros: {len(order.passengers)} (último mais próximo do destino)")
        ...
        print("\n🚀 Passo 5: Enviando para API MinasTaxi...")
        ...
        print("\n" + "="*60)
        print("✅ SUCESSO! PEDIDO ENVIADO (ROTA OTIMIZADA)")
        print("="*60)
